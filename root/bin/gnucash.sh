...
SECURITY_GROUP=$(uuidgen) &&
        ...
        rm -rf /opt/docker/workspace/lieutenant &&
            ...
            aws \
                ...
                --instance-ids $(aws \
                    ec2 \
                    terminate-instances \
                    --instance-ids $(aws ec2 describe-instance --filters Name=tag:moniker,Values=lieutenant Name=instance-state-name,Values=running --query "Reservations[0].Instances[*].InstanceId" --output text) \
    ...
    ssh lieutenant-ec2 sudo mount /dev/xvdh /data &&
